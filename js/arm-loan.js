...
function createARMLoanFields() {
    ...
    // Create common fields container (interest rate will show as "Fully Indexed Rate")
    createCommonFieldsContainer('arm');
    ...
    try {
        // ARM-specific fields for tab container
        const armSpecificFields = [
            // ARM Index (dropdown with external links)
            (() => {
                const indexOptions = [
                ...
                return createSelectElement('armIndex', 'ARM Index', indexOptions, '');
            })(),
            createInputElement('indexRate', 'number', 'Current Index Rate (%)', '', 'Enter the current rate from your selected index', 0, 50, 0.01),
            ...
            (() => {
                ...
                return createSelectElement('fixedPeriod', 'Fixed Rate Period', fixedPeriodOptions, 5);
            })(),
            (() => {
                ...
                return createSelectElement('adjustmentPeriod', 'Adjustment Period', adjustmentOptions, 12);
            })(),
        ...
        // Add ARM-specific fields to tab container
        armSpecificFields.forEach(field => {
            if (field) {
                tabContainer.appendChild(field);
            }
        });
        
        // Set up event listeners for ARM-specific fields
        ...
        addCommonFieldEventListeners(calculateARMLoan);
...
function updateFullyIndexedRate() {
    ...
    // Sync this rate to the separate ARM fully indexed rate field (don't affect other tabs)
    synchronizeField('armFullyIndexedRate', fullyIndexedRate.toFixed(3));
...
function generateARMAmortizationSchedule(principal, initialRate, months, startDate, firstPaymentDate, paymentDueDay, extraPayment = 0) {
    const schedule = [];
