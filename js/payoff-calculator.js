...
function createPayoffCalculatorSpecificFields() {
    ...
    const principalBalanceField = createInputElement('principalBalanceOnPED', 'currency', 'Principal Balance on PED ($)', 0, '', 0, null, 1);
    ...
    const finalPaymentField = createInputElement('finalPaymentBeforePayoff', 'date', 'Final Payment Before Payoff', '', 'Auto-calculated');
    const interimPrincipalField = createInputElement('interimPrincipalPaydown', 'currency', 'Interim Principal Paydown ($)', 0, 'Auto-calculated');
    const dailyInterestField = createInputElement('dailyInterestAmount', 'currency', 'Daily Interest Amount ($)', 0, 'Auto-calculated');
    const accruedInterestField = createInputElement('accruedInterestAtPayoff', 'currency', 'Accrued Interest at Payoff ($)', 0, 'Auto-calculated');
...
function calculatePayoffScenario() {
    ...
    const principalBalance = getNumericValue('principalBalanceOnPED');
    ...
    // Set calculated values with proper currency formatting
    setCurrencyValue('interimPrincipalPaydown', interimPrincipalPaydown);
    setCurrencyValue('dailyInterestAmount', dailyInterest);
    setCurrencyValue('accruedInterestAtPayoff', accruedInterest);
...
function updatePayoffSummary(principalBalance, interimPrincipalPaydown, accruedInterest, payoffDate, pedDate, finalPaymentDate) {
    ...
    // Format all dates consistently using the same format
    ...
    summarySection.innerHTML = `
        ...
        <div style="background: white; border-radius: 8px; padding: 1rem; margin-bottom: 1rem; border: 1px solid #e5e7eb;">
            ...
            <p style="font-size: 0.85rem; background: #f9fafb; padding: 0.5rem; border-radius: 4px; margin: 0; line-height: 1.4;">
        ...
        <div style="background: white; border-radius: 8px; padding: 1rem; border: 1px solid #e5e7eb;">
            ...
            <p style="font-size: 1rem; font-weight: bold; color: #1e40af; margin: 0;">
                Payoff Amount on ${formattedPayoffDate}: ${formatCurrency(payoffAmount)}
            </p>
            <p style="font-size: 0.9rem; color: #6b7280; margin-top: 0.5rem; margin-bottom: 0;">
...
function updateDefaultPrincipalBalance() {
    ...
    // Set value with proper currency formatting
    setCurrencyValue('principalBalanceOnPED', balanceOnPED);
}
...
function setCurrencyValue(elementId, value) {
    const element = document.getElementById(elementId);
    if (element) {
        // Format the value with commas (same as formatCurrencyInput function does)
