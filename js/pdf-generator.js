...
function addChartImage(doc, chartImage, margin, contentWidth, yPos, borrowerName = '') {
    ...
    // Check if image fits on current page (more space available after removing middle table)
...
function addRecastingWarning(doc, margin, contentWidth, yPos) {
    // Get current loan values for calculations - use correct field names
    const loanAmountElement = document.getElementById('loanAmount');
    const interestRateElement = document.getElementById('interestRate');
    const loanTermYearsElement = document.getElementById('loanTermYears');
    const loanTermMonthsElement = document.getElementById('loanTermMonths');
    ...
    const loanTermYears = loanTermYearsElement ? parseInt(loanTermYearsElement.value) || 0 : 0;
    const loanTermMonths = loanTermMonthsElement ? parseInt(loanTermMonthsElement.value) || 0 : 0;
    const loanTerm = loanTermYears + (loanTermMonths / 12);
...
function addBalloonWarning(doc, margin, contentWidth, yPos) {
    ...
    const loanTermYearsElement = document.getElementById('loanTermYears');
    const loanTermMonthsElement = document.getElementById('loanTermMonths');
    const balloonTermYearsElement = document.getElementById('balloonTermYears');
    const balloonTermMonthsElement = document.getElementById('balloonTermMonths');
    ...
    const loanTermYears = loanTermYearsElement ? parseInt(loanTermYearsElement.value) || 0 : 0;
    const loanTermMonths = loanTermMonthsElement ? parseInt(loanTermMonthsElement.value) || 0 : 0;
    const amortizationPeriod = loanTermYears + (loanTermMonths / 12);
    const balloonTermYears = balloonTermYearsElement ? parseInt(balloonTermYearsElement.value) || 0 : 0;
    const balloonTermMonths = balloonTermMonthsElement ? parseInt(balloonTermMonthsElement.value) || 0 : 0;
    const balloonTerm = balloonTermYears + (balloonTermMonths / 12);
...
function getCurrentInputValues() {
    const inputs = {};
    // Get inputs from all containers: common fields, tab-specific fields, and any standalone inputs
    const inputElements = document.querySelectorAll(
        '#commonFieldsContainer input, #commonFieldsContainer select, ' +
        '#tabSpecificFieldsContainer input, #tabSpecificFieldsContainer select, ' +
        'input, select'
    );
    
    inputElements.forEach(element => {
        // Only save inputs that have an ID (avoid duplicates and unnamed elements)
        if (element.id) {
    ...
    // Also save shared field values to ensure synchronization
    if (window.sharedFieldValues) {
        inputs.sharedFieldValues = window.sharedFieldValues;
    }
...
async function loadCalculationData(file) {
    try {
        ...
        setTimeout(() => {
            // Restore shared field values first if they exist
            if (data.inputs && data.inputs.sharedFieldValues) {
                window.sharedFieldValues = { ...window.sharedFieldValues, ...data.inputs.sharedFieldValues };
            }
            
            // Restore input values
            Object.entries(data.inputs || {}).forEach(([key, value]) => {
                // Skip the sharedFieldValues object (we handled it above)
                if (key === 'sharedFieldValues') return;
                ...
                if (element) {
                    ...
                    // Trigger synchronization for synchronized fields
                    if (window.sharedFieldValues && window.sharedFieldValues.hasOwnProperty(key)) {
                        window.sharedFieldValues[key] = value;
                        // Trigger input event to sync field across tabs
        ...
        }, 200); // Increased timeout to allow for field synchronization
...
async function createPayoffCalculatorPDFDocument() {
    ...
    // Skip the redundant tab-specific fields table since Present Balance and Payoff Date are covered in Payoff Summary
    
    // Loan Summary Table
    yPos = addPayoffCalculatorLoanSummaryTable(doc, summary, margin, contentWidth, yPos);
    
    // Payoff Summary Section
    yPos = addPayoffCalculatorPayoffSummary(doc, summary, margin, contentWidth, yPos);
...
function addPayoffCalculatorLoanSummaryTable(doc, summary, margin, contentWidth, yPos) {
    ...
    return row3Y + rowHeight + 4; // Reduced spacing to make room for chart
}

// Add payoff summary section for payoff calculator PDF
function addPayoffCalculatorPayoffSummary(doc, summary, margin, contentWidth, yPos) {
    // Get payoff summary data from the webapp
    const pedDate = document.getElementById('payoffEstimateDate')?.value || '';
    const payoffDate = document.getElementById('payoffDate')?.value || '';
    const principalBalance = getNumericValue('principalBalanceOnPED') || 0;
    const interimPrincipalPaydown = parseFloat(document.getElementById('interimPrincipalPaydown')?.value?.replace(/,/g, '') || 0);
    const accruedInterest = parseFloat(document.getElementById('accruedInterestAtPayoff')?.value?.replace(/,/g, '') || 0);
    const finalPaymentDate = document.getElementById('finalPaymentBeforePayoff')?.value || '';
    ...
    const payoffDateObj = new Date(payoffDate + 'T00:00:00');
    const pedDateObj = new Date(pedDate + 'T00:00:00');
    
    const formattedPayoffDate = isNaN(payoffDateObj.getTime()) ? payoffDate : payoffDateObj.toLocaleDateString('en-US', dateFormatOptions);
    const formattedPEDDate = isNaN(pedDateObj.getTime()) ? pedDate : pedDateObj.toLocaleDateString('en-US', dateFormatOptions);
    
    let formattedFinalPaymentDate = 'N/A';
    if (finalPaymentDate) {
        const finalPaymentDateObj = new Date(finalPaymentDate + 'T00:00:00');
        formattedFinalPaymentDate = isNaN(finalPaymentDateObj.getTime()) ? finalPaymentDate : finalPaymentDateObj.toLocaleDateString('en-US', dateFormatOptions);
    }
    ...
    doc.setFontSize(10);
    doc.setFont(undefined, 'bold');
    const headerText = 'Payoff Summary';
    const headerWidth = doc.getTextWidth(headerText);
    doc.text(headerText, tableX + (tableWidth - headerWidth) / 2, headerY + 8);
    
    // Formula section
    const formulaY = headerY + rowHeight;
    const formulaHeight = 18; // Taller for formula text
    doc.rect(tableX, formulaY, tableWidth, formulaHeight);
    doc.setFontSize(8);
    doc.setFont(undefined, 'bold');
    doc.text('Formula:', tableX + 5, formulaY + 6);
    doc.setFont(undefined, 'normal');
    
    // Split formula text to fit in PDF width
    const formulaText = `Payoff Amount on ${formattedPayoffDate} = Principal Balance on ${formattedPEDDate} - Principal Paydown (${formattedPEDDate} to ${formattedPayoffDate}) + Accrued Interest (${formattedFinalPaymentDate} to ${formattedPayoffDate})`;
    
    // Use text wrapping for long formula
    const maxWidth = tableWidth - 10; // Account for margins
    const lines = doc.splitTextToSize(formulaText, maxWidth);
    let lineY = formulaY + 12;
    lines.forEach(line => {
        doc.text(line, tableX + 5, lineY);
        lineY += 4;
    });
    
    // Calculation section
    const calcY = formulaY + formulaHeight;
    const calcHeight = 20; // Height for calculation
    doc.rect(tableX, calcY, tableWidth, calcHeight);
    doc.setFont(undefined, 'bold');
    doc.text('Calculation:', tableX + 5, calcY + 6);
    doc.setFont(undefined, 'normal');
    
    // Main result
    doc.setFont(undefined, 'bold');
    doc.text(`Payoff Amount on ${formattedPayoffDate}: ${formatCurrency(payoffAmount)}`, tableX + 5, calcY + 12);
    
    // Breakdown calculation
    doc.setFont(undefined, 'normal');
    doc.setFontSize(7);
    const breakdownText = `${formatCurrency(principalBalance)} - ${formatCurrency(interimPrincipalPaydown)} + ${formatCurrency(accruedInterest)} = ${formatCurrency(payoffAmount)}`;
    doc.text(breakdownText, tableX + 5, calcY + 16);
    
    return calcY + calcHeight + 4; // Reduced spacing to make room for chart
}

function getPayoffCalculatorSummaryData() {
    // Get live data directly from the webapp fields
    const principalBalance = getNumericValue('principalBalanceOnPED') || 0;
    const interimPrincipalPaydown = parseFloat(document.getElementById('interimPrincipalPaydown')?.value?.replace(/,/g, '') || 0);
    const accruedInterest = parseFloat(document.getElementById('accruedInterestAtPayoff')?.value?.replace(/,/g, '') || 0);
    const dailyInterest = parseFloat(document.getElementById('dailyInterestAmount')?.value?.replace(/,/g, '') || 0);
    const regularPayments = parseInt(document.getElementById('regularPaymentsUntilPayoff')?.value || 0);
    const payoffDate = document.getElementById('payoffDate')?.value || 'Not Specified';
    const finalPaymentDate = document.getElementById('finalPaymentBeforePayoff')?.value || '';
    
    // Calculate total payoff amount
    const totalPayoffAmount = principalBalance - interimPrincipalPaydown + accruedInterest;
    
    // Calculate days to payoff
    let daysToPayoff = 'Not Calculated';
    if (finalPaymentDate && payoffDate) {
        try {
            const finalDate = new Date(finalPaymentDate);
            const payoffDateObj = new Date(payoffDate);
            const daysDiff = Math.ceil((payoffDateObj - finalDate) / (1000 * 60 * 60 * 24));
            daysToPayoff = `${daysDiff} days`;
        } catch (e) {
            daysToPayoff = 'Not Calculated';
        }
    }
    
    // Calculate balance after last payment (which is the principal balance minus interim paydown)
    const balanceAfterPayments = principalBalance - interimPrincipalPaydown;
    
    return {
        currentBalance: principalBalance,
        principalPaydown: interimPrincipalPaydown,
        balanceAfterPayments: balanceAfterPayments,
