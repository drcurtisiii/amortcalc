...
function createStandardLoanFields() {
    ...
    if (extraPaymentField) {
        ...
        if (inputElement) {
            inputElement.addEventListener('input', calculateStandardLoan);
        }
    }
    
    // Add event listeners to common fields
    addCommonFieldEventListeners(calculateStandardLoan);
...
function calculateStandardLoan() {
    const values = getStandardFormValues();
...
function getStandardFormValues() {
    // Get standard loan form values using correct field names
